...
type inspectRulesInformer struct {
...
func (f *inspectRulesInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredInspectRulesInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}
...
func (f *inspectRulesInformer) Lister() v1alpha2.InspectRulesLister {
	return v1alpha2.NewInspectRulesLister(f.Informer().GetIndexer())
}

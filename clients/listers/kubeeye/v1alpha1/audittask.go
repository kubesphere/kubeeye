...
type AuditTaskLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.AuditTask, err error)
...
// List lists all AuditTasks in the indexer.
func (s *auditTaskLister) List(selector labels.Selector) (ret []*v1alpha1.AuditTask, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.AuditTask))
	})
	return ret, err
}
...
type AuditTaskNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.AuditTask, error)
...
// List lists all AuditTasks in the indexer for a given namespace.
func (s auditTaskNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.AuditTask, err error) {
...
// Get retrieves the AuditTask from the indexer for a given namespace and name.
func (s auditTaskNamespaceLister) Get(name string) (*v1alpha1.AuditTask, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("audittask"), name)
	}
	return obj.(*v1alpha1.AuditTask), nil
}

...
type AuditPlanLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.AuditPlan, err error)
...
// List lists all AuditPlans in the indexer.
func (s *auditPlanLister) List(selector labels.Selector) (ret []*v1alpha2.AuditPlan, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.AuditPlan))
	})
	return ret, err
}
...
type AuditPlanNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.AuditPlan, error)
...
// List lists all AuditPlans in the indexer for a given namespace.
func (s auditPlanNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.AuditPlan, err error) {
...
// Get retrieves the AuditPlan from the indexer for a given namespace and name.
func (s auditPlanNamespaceLister) Get(name string) (*v1alpha2.AuditPlan, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("auditplan"), name)
	}
	return obj.(*v1alpha2.AuditPlan), nil
}

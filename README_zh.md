...
KubeEye 支持自定义巡检规则、插件安装，通过 [KubeEye Operator](#kubeeye-operator) 能够使用 web 页面的图形化展示来查看巡检结果以及给出的修复建议。
...
- [可选] 安装 [Node-problem-Detector](https://github.com/kubernetes/node-problem-detector)
  注意：这将在你的集群上安装 npd，只有当你想要详细的节点报告时才需要安装。
```shell
kubeeye install npd
```
...
- KubeEye 可以帮助你检测各种集群节点问题，包括内存/CPU/磁盘压力，意外的内核错误日志等。
